...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(val, key) in bookStores?.storeTypes" :key="key">{{ key }} : {{ val }}</li>
      </ul>
      ...
      <ul>
        <li v-for="(val, key) in bookStores?.openingHours" :key="key">
          {{ key }} : {{ val.open }} - {{ val.close }}
        </li>
      </ul>
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="country in bookStores?.countries" :key="country.id">
          {{ country }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
      <ul>
        <li v-for="seller in bookStores?.topSellers" :key="seller.id">
          {{ seller }}
        </li>
      </ul>
    </section>
...
const orwell = computed(() => authors.find(({ name }) => name === 'George Orwell'))
